...
export default function Hero() {
  const { t } = useLang();
  return (
    <section className="relative w-full min-h-[100vh] bg-[#000000] text-[#ffffff] overflow-hidden flex flex-col">
      ...
      <aside aria-hidden className="hidden md:block absolute right-4 top-1/2 -translate-y-1/2 rotate-90 origin-right text-[10px] font-medium uppercase tracking-normal text-[#ffffff]">
        MUSE WEAR 1-MODEL
      </aside>

      <div className="relative flex-1 w-full px-6 md:px-10 pt-44 pb-10 flex flex-col justify-end">
        <div className="text-[12px] font-medium uppercase text-[#ffffff]">{t("MADE FOR MOTION, BUILT IN CAIRO.", "صُنع للحركة، بُني في القاهرة.")}</div>
        <h1 className="oryzo-display mt-4 max-w-4xl text-left">
          MUSE
          <br />
          WEAR
        </h1>
        <p className="oryzo-body mt-6 max-w-2xl text-left text-[#ffffff]">
        ...
        <div className="mt-10 grid md:grid-cols-[320px_1fr_220px] gap-[18px] items-end">
          <div className="card-oryzo bg-[#000000]/60">
            <div className="oryzo-label text-left">DESIGNED IN CAIRO, THE HYBRID HOUSE.</div>
            <hr className="divider-dashed my-3" />
            <p className="text-[14px] leading-relaxed font-medium uppercase text-left text-[#ffffff]/90">
              {t("MULTIBRAND MARKETPLACE + OUR OWN FACTORY.", "سوق متعدد الماركات + مصنعنا الخاص.")}
            </p>
          </div>
          <div className="hidden md:block" />
          <Link href="/muse" className="card-oryzo text-left hover:border-[#ffffff] transition">
            <div className="oryzo-label">MUSE MANUFACTURED ▶</div>
            <div className="oryzo-legal mt-2 text-[#737373]">* CAIRO FACTORY DROP 2026</div>
          </Link>
        </div>
      </div>
    </section>
