    ...
    <header className={`sticky top-0 z-50 bg-white border-b transition-shadow ${scrolled ? "shadow-[0_2px_20px_rgba(0,0,0,0.06)] border-black/10" : "border-black/10"}`}>
      ...
      <div className="max-w-[1400px] mx-auto px-4">
        <div className="flex items-center gap-4 py-3">
          ...
          <div className="flex-1 max-w-2xl hidden md:flex">
            <div className="flex w-full border border-zinc-300 rounded-full overflow-hidden focus-within:border-black transition-colors bg-zinc-50 focus-within:bg-white">
              <input placeholder={t("Search for products, brands and more", "ابحث عن منتجات، ماركات وأكثر")} className="flex-1 px-5 py-2.5 text-sm outline-none bg-transparent" />
              <button className="px-6 bg-black text-white text-sm font-semibold hover:bg-zinc-800 transition"> {t("Search", "بحث")}</button>
            </div>
          </div>

          <div className="flex items-center gap-1 sm:gap-3 ml-auto text-sm">
            <button onClick={toggle} className="hidden sm:inline-flex border border-zinc-300 rounded-full px-3 py-1.5 font-semibold hover:border-black hover:bg-black hover:text-white transition">
              {lang === "en" ? "العربية" : "English"}
            </button>
            <Link href="/wishlist" className="relative p-2 hover:scale-110 transition">
            ...
            <Link href="/cart" className="relative p-2 hover:scale-110 transition">
              <span className="text-xl">🛒</span>
        ...
        <nav className="flex gap-6 text-sm font-bold tracking-wide overflow-x-auto no-scrollbar border-t border-zinc-100 py-3">
          <Link href="/women" className="whitespace-nowrap hover:text-black text-zinc-700 relative after:absolute after:left-0 after:-bottom-1 after:h-[2px] after:w-0 hover:after:w-full after:bg-black after:transition-all"> {t("WOMEN", "نسائي")} </Link>
          <Link href="/men" className="whitespace-nowrap hover:text-black text-zinc-700 relative after:absolute after:left-0 after:-bottom-1 after:h-[2px] after:w-0 hover:after:w-full after:bg-black after:transition-all"> {t("MEN", "رجالي")} </Link>
          <Link href="/accessories" className="whitespace-nowrap hover:text-black text-zinc-700 relative after:absolute after:left-0 after:-bottom-1 after:h-[2px] after:w-0 hover:after:w-full after:bg-black after:transition-all"> {t("ACCESSORIES", "إكسسوارات")} </Link>
          <span className="text-zinc-300">|</span>
          <Link href="/women" className="whitespace-nowrap text-zinc-600 font-semibold hover:text-black"> {t("New In", "وصل حديثاً")} </Link>
          <Link href="/men" className="whitespace-nowrap text-zinc-600 font-semibold hover:text-black"> {t("MUSE Manufactured", "صناعة موس")} </Link>
          <span className="bg-red-600 text-white px-2 py-0.5 rounded text-xs whitespace-nowrap animate-pulse"> {t("SALE", "تخفيضات")} </span>
        </nav>
      </div>
    </header>
  );
